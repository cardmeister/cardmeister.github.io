(function(){'use strict';//SVGcardt

let createElementCardT = CARDTS => {
    let ranks_suits = [...CARDTS.ranks, ...CARDTS.suits];
    let drawCard = cardt => {
        let doc = 'ATTRs:\n';
        let newcard = CARDTS.SVGcardt(cardt.constructor.observedAttributes
            .reduce((settings, attr) => {
                if (cardt.hasAttribute(attr)) {
                    let value = cardt.getAttribute(attr);
                    settings[attr] = value || attr;
                    if (ranks_suits.includes(attr)) doc += attr + '  ';
                    else doc += `\n${attr} : ${value}`;
                }
                return settings;
            }, { cardcolor: '#FEFEFE' }
    ...
    customElements.define('card-t', class extends HTMLElement {
        static get observedAttributes() {
            return [...ranks_suits, ...CARDTS.attributes];
        }
        constructor() {
            super();
        }
